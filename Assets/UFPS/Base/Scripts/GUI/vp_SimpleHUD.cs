...
public class vp_SimpleHUD : MonoBehaviour
{
	...
	vp_FPPlayerDamageHandler playerDH;
	private vp_PlayerInventory playerInventory;
	public Font BigFont;
	...
	public Texture2D ArmorIcon = null;
	...
	protected float m_ArmorWidth { get { return ((HealthStyle.CalcSize(new GUIContent(FormattedArmor)).x)); } }
	...
	void Start()
	{
		...
		playerDH = GetComponent<vp_FPPlayerDamageHandler>();
		playerInventory = GetComponent<vp_PlayerInventory>();
	}
	...
	string FormattedHealth
	{
		get
		{
			...
			if (m_FormattedHealth > 100f)
				m_FormattedHealth = 100f;
			else if (m_FormattedHealth < 0f)
	...
	string FormattedArmor
	{
		get
		{
			float armor = playerDH.currentArmor * HealthMultiplier;
	...
	void DrawHealth()
	{
		DrawLabel("", new Vector2(m_CurrentHealthOffset, Screen.height - 68), new Vector2(200 + m_HealthWidth, 52), AmmoStyle, Color.white, m_TranspBlack, null);   // background
		if (HealthIcon != null)
		...
		if (playerDH.currentArmor > 0)
		{
			if (ArmorIcon != null)
				DrawLabel("", new Vector2(m_CurrentHealthOffset + m_HealthWidth + 80, Screen.height - 58), new Vector2(32, 32), AmmoStyle, Color.blue, HealthColor, ArmorIcon);         // icon

			DrawLabel(FormattedArmor, new Vector2(m_CurrentHealthOffset + m_HealthWidth + m_ArmorWidth + 12, Screen.height - BigFontOffset), new Vector2(110, 60), HealthStyle, Color.blue, Color.clear, null);
			DrawLabel("%", new Vector2(m_CurrentHealthOffset + 125 + m_HealthWidth + m_ArmorWidth, Screen.height - SmallFontOffset), new Vector2(110, 60), AmmoStyleSmall, Color.blue, Color.clear, null);  // percentage mark
		}
