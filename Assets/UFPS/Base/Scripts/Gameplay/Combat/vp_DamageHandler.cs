...
public class vp_DamageHandler : MonoBehaviour
{
	...
	{		
        ...
        if (!Instances.ContainsKey(GetComponent<Collider>()))
	...
	public virtual void Damage(vp_DamageInfo damageInfo)
	{
		...
		if (damageInfo != null)
		{
            ...
            if (Transform.name != "Hero" && Transform.name.Contains("BOSS") == false)
            {
                GameObject blood = Instantiate(Resources.Load("Particle/BloodSplatter" + UnityEngine.Random.Range(1, 12)) as GameObject);
                blood.transform.position = Transform.position;
                blood.transform.localPosition = new Vector3(blood.transform.localPosition.x, UnityEngine.Random.Range(2.8f, 3.6f), blood.transform.localPosition.z);
                Destroy(blood, 2);
            }
        }
		...
		vp_FPPlayerDamageHandler hero = GetComponent<vp_FPPlayerDamageHandler>();

		if(hero && hero.currentArmor > 0)
			hero.currentArmor = Mathf.Min(hero.currentArmor - damageInfo.Damage, MaxHealth);
		else
	...
	public virtual void Die()
	{
        ...
        RespawnControl.Instance.SendMessage("OnOneZombieDie", GetComponent<ZombieBehaviour>(), SendMessageOptions.DontRequireReceiver);
