...
public class vp_Shooter : vp_Component
{
    ...
    /// </summary>
    /// 
    private int m_AxeUse = 20;
    private int m_BaseballBatUse = 20;
    ...
    protected virtual void Fire()
	{
        switch (ProjectilePrefab.name)
        {
            case "PistolBullet":
                {
                    ...
                    float reduced = newLevel * WeaponData.dataMap[1].GetFireRate_LevelUp();
                    float fireRate = WeaponData.dataMap[1].GetFireRate() - reduced;
                    ProjectileFiringRate = fireRate;
                }
                break;
            case "ShotgunBullet":
                {
                    ...
                    int newLevel = WeaponInventory.levelList[inventory_id] - 1;
                    float reduced = newLevel * WeaponData.dataMap[2].GetFireRate_LevelUp();
                    float fireRate = WeaponData.dataMap[2].GetFireRate() - reduced;
                    ProjectileFiringRate = fireRate;
                }
                break;
            case "MachinegunBullet":
                {
                    ...
                    int newLevel = WeaponInventory.levelList[inventory_id] - 1;
                    float reduced = newLevel * WeaponData.dataMap[3].GetFireRate_LevelUp();
                    float fireRate = WeaponData.dataMap[3].GetFireRate() - reduced;
                    ProjectileFiringRate = fireRate;
                }
                break;
            case "GrenadegunBullet":
                {
                    int inventory_id = WeaponInventory.GetInventoryID(3);
                    int newLevel = WeaponInventory.levelList[inventory_id] - 1;
                    float reduced = newLevel * WeaponData.dataMap[4].GetFireRate_LevelUp();
                    float fireRate = WeaponData.dataMap[4].GetFireRate() - reduced;
                    ProjectileFiringRate = fireRate;
                }
                break;
            case "SuperrifleBullet":
                {
                    int inventory_id = WeaponInventory.GetInventoryID(4);
                    int newLevel = WeaponInventory.levelList[inventory_id] - 1;
                    float reduced = newLevel * WeaponData.dataMap[5].GetFireRate_LevelUp();
                    float fireRate = WeaponData.dataMap[5].GetFireRate() - reduced;
                    ProjectileFiringRate = fireRate;
                }
                break;
            case "CrossBowArrow":
                {
                    ...
                    int newLevel = WeaponInventory.levelList[inventory_id] - 1;
                    float reduced = newLevel * WeaponData.dataMap[6].GetFireRate_LevelUp();
                    float fireRate = WeaponData.dataMap[6].GetFireRate() - reduced;
                    ProjectileFiringRate = fireRate;
                }
                break;
            case "XenonBullet":
                {
                    ...
                    int newLevel = WeaponInventory.levelList[inventory_id] - 1;
                    float reduced = newLevel * WeaponData.dataMap[7].GetFireRate_LevelUp();
                    float fireRate = WeaponData.dataMap[7].GetFireRate() - reduced;
                    ProjectileFiringRate = fireRate;
                }
                break;
			case "Flame":
				{
					...
					int newLevel = WeaponInventory.levelList[inventory_id] - 1;
					float reduced = newLevel * WeaponData.dataMap[8].GetFireRate_LevelUp();
					float fireRate = WeaponData.dataMap[8].GetFireRate() - reduced;
					ProjectileFiringRate = fireRate;
				}
				break;
			case "AxeAttack":
            ...
            case "BaseBallAttack":
                {
                    if (m_BaseballBatUse <= 0)
                        return;

                    //int inventory_id = WeaponInventory.GetInventoryID(8);
                    //int newLevel = WeaponInventory.levelList[inventory_id] - 1;
                    //float reduced = newLevel * WeaponData.dataMap[9].GetFireRate_LevelUp();
                    //float fireRate = WeaponData.dataMap[9].GetFireRate() - reduced;
                    //ProjectileFiringRate = fireRate;
                    m_BaseballBatUse--;
                }
                break;
		...
		if(HaveFireMessage && GameSetting.soundOn == true)
