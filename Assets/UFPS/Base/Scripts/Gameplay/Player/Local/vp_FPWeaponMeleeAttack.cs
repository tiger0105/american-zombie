...
public class vp_FPWeaponMeleeAttack : vp_Component
{
    ...
    protected void UpdateAttack()
	{
        ...
        if (transform.name.Contains("Axe") && GameObject.Find("Hero").GetComponent<vp_WeaponHandler>().m_AxeUse <= 0
            || transform.name.Contains("BaseballBat") && GameObject.Find("Hero").GetComponent<vp_WeaponHandler>().m_BaseballBatUse <= 0)
            return;
		...
		vp_Timer.In(SwingDelay, delegate()
		{
			...
			vp_Timer.In(ImpactTime, delegate()
			{
                ...
                if (transform.name.Contains("BaseballBat"))
                {
                    GameObject.Find("Hero").GetComponent<vp_WeaponHandler>().m_BaseballBatUse--;
                    if (GameObject.Find("Hero").GetComponent<vp_WeaponHandler>().m_BaseballBatUse >= 0)
                        GameObject.Find("RangeAttackCount").GetComponentInChildren<Text>().text 
                        = GameObject.Find("Hero").GetComponent<vp_WeaponHandler>().m_BaseballBatUse.ToString();
                }
                else if (transform.name.Contains("Axe"))
                {
                    GameObject.Find("Hero").GetComponent<vp_WeaponHandler>().m_AxeUse--;
                    if (GameObject.Find("Hero").GetComponent<vp_WeaponHandler>().m_AxeUse >= 0)
                        GameObject.Find("RangeAttackCount").GetComponentInChildren<Text>().text 
                        = GameObject.Find("Hero").GetComponent<vp_WeaponHandler>().m_AxeUse.ToString();
                }
